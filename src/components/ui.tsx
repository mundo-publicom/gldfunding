  ...
  return (
    <div
      className={cn(
        ...
        invert ? 'border-mint-glow' : 'border-mint',
      )}
    >
      <p
        className={cn(
    ...
    <div>
      <div
        className={cn(
          ...
          invert ? 'text-mint-glow' : 'text-mint-deep',
        )}
      >
        {value}
      </div>
...
export function FeatureCard({
  ...
  return (
    <div className="bg-white p-6 lg:p-7">
      {icon && <div className="mb-4 text-mint-deep">{icon}</div>}
  ...
  return (
    <div
      className={cn(
        ...
        '[&_li]:mt-2 [&_li]:marker:text-mint',
        '[&_strong]:font-semibold [&_strong]:text-ink',
        '[&_a]:text-mint-deep [&_a]:underline [&_a]:underline-offset-[3px] [&_a]:decoration-1',
        className,
      )}
    >
      {children}
  ...
  return (
    <div
      className={cn(
        ...
        tone === 'accent' ? 'border-mint bg-paper' : 'border-rate bg-rate-bg',
      )}
    >
        ...
        {trail.map((t, i) => (
          <li key={t.path} className="flex items-center gap-1.5">
            ...
            {i === trail.length - 1 ? (
              ...
              <Link to={t.path} className="transition-colors hover:text-mint-deep">
                {t.name}
              </Link>
            )}
          </li>
      ...
      {items.map((item) => (
        <details key={item.q} className="group py-5">
          <summary className="flex cursor-pointer list-none items-start justify-between gap-6 text-[1.0625rem] font-medium text-ink [&::-webkit-details-marker]:hidden">
            {item.q}
            <span className="relative mt-2 h-[1px] w-3.5 shrink-0 bg-ink-3 transition-colors group-open:bg-mint-deep">
